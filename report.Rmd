...
mfi_vs_concentration_plot(params$raw_upload_data)
mfi_vs_time_plot(params$raw_upload_data)
stain_index(params$raw_upload_data)
signal_to_noise(params$raw_upload_data)
percent_positive(params$raw_upload_data)
...
# if(is.na(full_data_df$Conc_15_ng)){
#     names(full_data_df) <- c("Time (yr)", "30 ng/test", "60 ng/test", "125 ng/test", "250 ng/test", "500 ng/test", "1000 ng/test", "2000 ng/test")
# }
# names(full_data_df)[1:9] <- c("Time (yr)", "30 ng/test", "60 ng/test", "125 ng/test", "250 ng/test", "500 ng/test", "1000 ng/test", "2000 ng/test")
kable(full_data_df, caption = '% of 4C MFI Reference Data at each Concentration')
# na.omit(params$raw_melted_data)
ggplot(params$raw_melted_data, aes(x=Time, y=value, color=Concentrations)) +
            ...
            geom_point(size=4) + # plot with keep dataset points filled in only
            labs(x = "Time (years)",
...
# _**Residuals vs. Fit Scatterplot:**_
```{r, message = FALSE, echo = FALSE, warning = FALSE}

# residual_vs_fit_plot(params$keep, params$poly_order)
params$resid_plot
# p <- ggplot(params$keep,aes(x=fit_residuals, label=Time)) + 
